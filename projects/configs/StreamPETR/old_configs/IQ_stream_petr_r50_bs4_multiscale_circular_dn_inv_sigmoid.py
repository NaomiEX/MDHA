...
encoder = dict(
    ...
    encode_query_with_ego_pos=True, # TODO: TRY FALSE
...
model = dict(
    ...
    ##new##
    mlvl_feats_format=mlvl_feats_format,
    extra_module=dn_model,
    # encoder=encoder,
    sparse_rho=0.8,
    process_backbone_mem=True,
    process_encoder_mem=True,
    use_mask_predictor=False,
    learn_ref_pts_type='embedding',
    num_cameras=6,
    pc_range=point_cloud_range,
    ret_enc_deform_attn_aux_outs=False, # return sampling locs and attn weights from encoder ms deform attn
    use_cam_embed=False,
    use_lvl_embed=False, # ! IMPORTANT: MAKE THIS TRUE IF USING ENCODER
    ##
    img_backbone=dict(
        type="ResNet",
        depth=50,
    ...
    #     two_stage=True,
    #     encoder_aux_layers=encoder['num_layers'] - 1,
    ...
    # train_cfg=dict(
    #     pts=train_cfg_obj_det3d,
    #     encoder=train_cfg_obj_det3d,
    #     )
    )
