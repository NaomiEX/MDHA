...
from projects.mmdet3d_plugin.models.utils.projections import Projections
@PIPELINES.register_module()
class FilterObjectOutOfFrame():
    def __init__(self):
        pass

    def __call__(self, results):
        print("!!WARNING USING FILTER OBJECT OUT OF FRAME!!")
        # imgs = results['img']
        # B=len(imgs)
        new_gt_bboxes=[]
        new_gt_labels=[]
        ...
        cam_transformations = dict(
            lidar2img=torch.tensor(results['lidar2img']).unsqueeze(0),
            lidar2cam=torch.tensor(results['extrinsics']).unsqueeze(0)
        )
        # print(f"lidar2img shape: {cam_transformations['lidar2img'].shape}\n"
        #       f"extrinsics shape: {cam_transformations['lidar2cam'].shape}")
        gt_bboxes = results['gt_bboxes_3d'].gravity_center.unsqueeze(0) # [n_obj, 3]
        # print(f"gt_bboxes:{results['gt_bboxes_3d'].tensor.shape}")

        projected_pts, projected_cams, non_matches = \
            Projections.project_to_2d_cam_point(gt_bboxes, cam_transformations,
                                                ret_non_matches=True)
        non_matches=non_matches.cpu().detach().numpy()
        # for i in range(B):
        # new_gt_bboxes.append()
        # new_gt_labels.append()
        # print(f"gt_labels_3d:{results['gt_labels_3d'].shape}")
        # print(f"non_matches:{non_matches.shape}")
        try:
            new_gt_bboxes = results['gt_bboxes_3d'][~non_matches[0]]
            new_gt_labels=results['gt_labels_3d'][~non_matches[0]]
        except:
            import pickle
            pickle.dump(results['gt_bboxes_3d'], open("./experiments/gt_bboxes_3d.pkl", "wb"))
            pickle.dump(results['gt_labels_3d'], open("./experiments/gt_labels_3d.pkl", "wb"))
            pickle.dump(non_matches, open("./experiments/non_matches.pkl", "wb"))
            print(results['gt_labels_3d'])
            print(non_matches)
            raise Exception()
        results['gt_bboxes_3d']=new_gt_bboxes
        results['gt_labels_3d']=new_gt_labels
