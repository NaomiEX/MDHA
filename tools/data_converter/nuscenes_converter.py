...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, test, max_sweeps=max_sweeps, cam_order=cam_order)
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # obtain 6 image's information per frame
        if cam_order is not None:
            assert isinstance(cam_order, (list, tuple)) and len(cam_order) == 6
            for cam_type in camera_types:
                assert cam_type in cam_order, f"{cam_type} not present in {cam_order}"
                assert cam_order.count(cam_type) == 1, f"{cam_type} appears {cam_order.count(cam_type)} times in {cam_order}"
            camera_types=cam_order
        ...
        if not test:
            ...
            # only take the yaw, so 1 val only
